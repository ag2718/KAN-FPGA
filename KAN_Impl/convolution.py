...
def calc_out_dims(matrix, kernel_size, stride, dilation, padding):
    batch_size,n_channels,n, m = matrix.shape

    h_out =  np.floor((n + 2 * padding[0] - kernel_size[0] - (kernel_size[0] - 1) * (dilation[0] - 1)) / stride[0]).astype(int) + 1
    w_out = np.floor((m + 2 * padding[1] - kernel_size[1] - (kernel_size[1] - 1) * (dilation[1] - 1)) / stride[1]).astype(int) + 1
...
def multiple_convs_kan_conv2d(matrix, #but as torch tensors. Kernel side asume q el kernel es cuadrado
    ...
    # print(f"Conv inputs: {kernel_size=}, {out_channels=}, {stride=}, {dilation=}, {padding=}")
    h_out, w_out,batch_size,n_channels = calc_out_dims(matrix, kernel_size, stride, dilation, padding)
    # print(f"Conv outupts: {h_out=}, {w_out=}, {batch_size=}, {n_channels=}")
    n_convs = len(kernels)
    ...
    conv_groups = unfold(matrix[:,:,:,:]).view(batch_size, n_channels,  kernel_size[0] * kernel_size[1], h_out*w_out).transpose(2, 3)#reshape((batch_size,n_channels,h_out,w_out))
