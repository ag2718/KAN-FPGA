...
class KANConv(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        #if self.n_convs>1:
        res = convolution.multiple_convs_kan_conv2d(x, self.convs,self.kernel_size,self.out_channels,self.stride,self.dilation,self.padding,self.device)
...
class KAN_Convolution(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        self.device = x.device
        return convolution.kan_conv2d(x, self.conv,self.kernel_size,self.stride,self.dilation,self.padding,self.device)
